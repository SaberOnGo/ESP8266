...
// Debug output.
#if HTTP_DEBUG_EN
#define PRINTF(...)  HTTP_DEBUG(__VA_ARGS__)
#else
#define PRINTF(...)
#endif
...
void ICACHE_FLASH_ATTR http_post(const char * url, const char * post_data, const char * headers, http_callback user_callback)
{
	...
	if (is_http)
		...
		HTTP_DEBUG("URL is not HTTP or HTTPS %s\n", url);
		return;
	}
...
{
	HTTP_DEBUG("http_status=%d\n", http_status);
	if (http_status != HTTP_STATUS_GENERIC_ERROR) {
		HTTP_DEBUG("strlen(headers)=%d\n", strlen(response_headers));
		HTTP_DEBUG("body_size=%d\n", body_size);
		HTTP_DEBUG("body=%s<EOF>\n", response_body); // FIXME: this does not handle binary data.
	}
}
