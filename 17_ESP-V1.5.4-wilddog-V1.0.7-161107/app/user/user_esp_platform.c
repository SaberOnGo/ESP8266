...
user_esp_platform_init(void)
{
	...
	IOT_VERSION_MINOR,IOT_VERSION_REVISION,device_type,UPGRADE_FALG);
	sys_debug("IOT VERSION = %s\n",iot_version);

	system_param_load(ESP_PARAM_START_SEC, 0, &esp_param, sizeof(esp_param));

	struct rst_info *rtc_info = system_get_rst_info();

	sys_debug("reset reason: %x\n", rtc_info->reason);

	if (rtc_info->reason == REASON_WDT_RST ||
		rtc_info->reason == REASON_EXCEPTION_RST ||
		rtc_info->reason == REASON_SOFT_WDT_RST) {
		if (rtc_info->reason == REASON_EXCEPTION_RST) {
			sys_debug("Fatal exception (%d):\n", rtc_info->exccause);
		}
		sys_debug("epc1=0x%08x, epc2=0x%08x, epc3=0x%08x, excvaddr=0x%08x, depc=0x%08x\n",
	...
	if(dhcp_info.flag == 0x01 ) {
		...
		if ( true != wifi_set_ip_info(STATION_IF,&sta_info)) {
			sys_debug("set default ip wrong\n");
		}
	}
