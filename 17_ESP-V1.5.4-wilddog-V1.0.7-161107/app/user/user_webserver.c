...
light_status_set(struct jsontree_context *js_ctx, struct jsonparse_state *parser)
{
    ...
    if((r|g|b|ww|cw) == 0){
        ...
        if(light_sleep_flg==1){
            WEBSERVER_DEBUG("modem sleep en\r\n");
...
LOCAL void ICACHE_FLASH_ATTR json_scan_cb(void *arg, STATUS status)
{
    ...
    while(bss != NULL) {
    	...
    	if(bss_temp == NULL) {
    		WEBSERVER_DEBUG("malloc scan info failed\n");
    		break;
    	} else{
...
upgrade_check_func(void *arg)
{
	...
	if(system_upgrade_flag_check() == UPGRADE_FLAG_START) {
        ...
        upgrade_lock = 0;
		WEBSERVER_DEBUG("local upgrade failed\n");
	} else if( system_upgrade_flag_check() == UPGRADE_FLAG_FINISH ) {
		WEBSERVER_DEBUG("local upgrade success\n");
...
{
    if (system_upgrade_flag_check() != UPGRADE_FLAG_START) {
    	WEBSERVER_DEBUG("system upgrade deinit\n");
        system_upgrade_deinit();
    }
}
...
local_upgrade_download(void * arg,char *pusrdata, unsigned short length)
{
    ...
    if (totallength == 0 && (ptr = (char *)os_strstr(pusrdata, "\r\n\r\n")) != NULL &&
		...
		if (ptr != NULL) {
			...
			if (ptmp2 != NULL) {
				...
				WEBSERVER_DEBUG("sumlength failed\n");
			}
		} else {
			WEBSERVER_DEBUG("Content-Length: failed\n");
		}
		if (sumlength != 0) {
        ...
        length -= 4;
        totallength += length;
        WEBSERVER_DEBUG("upgrade file download start.\n");
    ...
    if (totallength == sumlength) {
        WEBSERVER_DEBUG("upgrade file download finished.\n");
...
webserver_recv(void *arg, char *pusrdata, unsigned short length)
{
    ...
    if(upgrade_lock == 0){
        ...
        WEBSERVER_DEBUG("len:%u\n",length);
        if(check_data(pusrdata, length) == false)
        {
            WEBSERVER_DEBUG("goto\n");
             goto _temp_exit;
        }
...
//        os_printf(precvbuffer);
        ...
        switch (pURL_Frame->Type) {
            case GET:
                WEBSERVER_DEBUG("We have a GET request.\n");
            ...
            case POST:
                WEBSERVER_DEBUG("We have a POST request.\n");
                ...
                if (os_strcmp(pURL_Frame->pSelect, "config") == 0 &&
					...
					if (os_strcmp(pURL_Frame->pFilename, "start") == 0){
						response_send(ptrespconn, true);
						WEBSERVER_DEBUG("local upgrade start\n");
						upgrade_lock = 1;
						system_upgrade_init();
						...
						response_send(ptrespconn, true);
						WEBSERVER_DEBUG("local upgrade restart\n");
...
void webserver_recon(void *arg, sint8 err)
{
    ...
    WEBSERVER_DEBUG("webserver's %d.%d.%d.%d:%d err %d reconnect\n", pesp_conn->proto.tcp->remote_ip[0],
...
void webserver_discon(void *arg)
{
    ...
    WEBSERVER_DEBUG("webserver's %d.%d.%d.%d:%d disconnect\n", pesp_conn->proto.tcp->remote_ip[0],
        		pesp_conn->proto.tcp->remote_ip[1],pesp_conn->proto.tcp->remote_ip[2],
