...
//////**************RUN WHEN SLAVE RECIEVE*******************///////
   //tow lines below is to configure spi timing.
    ...
    SPI_DEBUG_PRINT("SPI_CTRL2 is %08x\n",READ_PERI_REG(SPI_CTRL2(spi_no)));
    ...
    spi_task(os_event_t *e)
{
    ...
    switch(e->sig){
       case MOSI:
            	...
            	break;
	case STATUS_R_IN_WR :
		SPI_DEBUG_PRINT("SR ERR in WRPR,Reg:%08x \n",e->par);
		break;
	case STATUS_W:
		SPI_DEBUG_PRINT("SW ERR,Reg:%08x\n",e->par);
		break;	
	case TR_DONE_ALONE:
		SPI_DEBUG_PRINT("TD ALO ERR,Reg:%08x\n",e->par);
		break;	
	case WR_RD:
		SPI_DEBUG_PRINT("WR&RD ERR,Reg:%08x\n",e->par);
		break;	
	case DATA_ERROR:
		SPI_DEBUG_PRINT("Data ERR,Reg:%08x\n",e->par);
		break;
	case STATUS_R_IN_RD :
		SPI_DEBUG_PRINT("SR ERR in RDPR,Reg:%08x\n",e->par);
		break;	
        default:
            break;
    }
...
void ICACHE_FLASH_ATTR
    spi_test_init()
{
    SPI_DEBUG_PRINT("spi init\n\r");
    spi_slave_init(HSPI);
    SPI_DEBUG_PRINT("gpio init\n\r");
    gpio_init();
    SPI_DEBUG_PRINT("spi task init \n\r");
    spi_task_init();
#ifdef SPI_MISO
    SPI_DEBUG_PRINT("spi miso init\n\r");
    set_miso_data();
#endif
