    ...
    hspi_master_dev_init(uint8 dev_no,uint8 clk_polar,uint8 clk_div)
{
	...
	{
		SPI_DEBUG_PRINT("hspi_master_dev_init parameter is out of range!\n\r");
		return;
	}
	...
	switch(dev_no){
		...
		case SPI_CS1_DEV :	
			...
			if(hspi_dev_reg.spi_io_80m){	
				SPI_DEBUG_PRINT("SPI CS1 device must work at 80Mhz");
			}
			break;
		...
		case SPI_CS2_DEV :	
			...
			if(hspi_dev_reg.spi_io_80m){	
				SPI_DEBUG_PRINT("SPI CS2 device must work at 80Mhz");
			}
			break;
    ...
    hspi_dev_sel(uint8 dev_no)
{
	...
	if(dev_no>3){
		SPI_DEBUG_PRINT("hspi_dev_sel parameter is out of range!\n\r");
		return;
	}

	if(!hspi_dev_reg.hspi_dev_conf[dev_no].active){
		SPI_DEBUG_PRINT("device%d has not been initialized!\n\r",dev_no);
		return;
	}
